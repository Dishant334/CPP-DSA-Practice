...
class Node{
    ...
    int SumTree(Node* root){
        ...
        int leftCount=SumTree(root->left);
        int rightCount=SumTree(root->right);

        return leftCount+rightCount+root->data;       //left count + right count + that head root
    }
int main(){
    vector<int>nodes={1,2,4,-1,-1,5,-1,-1,3,-1,6,-1,-1};
    Node *root=buildTrees(nodes);
 cout << "Root (head) of the tree: " << root->data << endl;
 cout<<"The count of the trees is "<<SumTree(root); 
}